	...
	if( mtrlBuffer != 0 && numMtrls != 0 )
	{
		...
		for(DWORD i = 0; i < numMtrls; ++i)
		{
			...
			if( d3dxmtrls[i].pTextureFilename != 0 )
			{
				...
				wchar_t unicodeTexFN[256];
				MultiByteToWideChar(CP_ACP, 0, texFN, -1, unicodeTexFN, 256);
				HR(D3DXCreateTextureFromFile(gd3dDevice, unicodeTexFN, &tex));
#else
				char* texFN = d3dxmtrls[i].pTextureFilename;
				HR(D3DXCreateTextureFromFile(gd3dDevice, _T(texFN), &tex));
#endif
