...
#pragma pack(1) // 1byte 정렬
	...
	typedef struct _tagLayerLoader
	{
		DWORD flag;					// CChunk::SLayer.flag 와 동일한 값이다.
		char alphaname[MAX_PATH];	// 알파 텍스쳐 파일이름
		char texturename[MAX_PATH];	// 텍스쳐 파일이름
	} SLayerLoader;

	typedef struct _tagSChunkLoader
	{
		int layerCount;
		SLayerLoader *pLayer;
	}SChunkLoader;

	typedef struct _tagSChunkGroupLoader
	{
		int rowChunkCount;
		int colChunkCount;
		int rowCellCountPerChunk;
		int colCellCountPerChunk;
		int alphaTextureSize; // 정사각형으로 가정함, 한 변의 픽셀 갯수
		int chunkCount; // pchunk 위에 선언되어야 한다.
		SChunkLoader *pchunk;
	} SChunkGroupLoader;
...
extern CMapView *g_pTerrainView;
