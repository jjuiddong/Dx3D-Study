...
void CFileLoader::Init( IDirect3DDevice9* pDevice ) 
{
	...
	if( !bLoaded )
	{
		s_MemLoader.LoadTokenFile( "data//script//def.txt" );
		bLoaded = TRUE;
	}
}
...
SBMMLoader* CFileLoader::LoadModel( char *szFileName )
{
	...
	ChangeExtendName(szFileName, strlen(szFileName), "bmm", binfile, sizeof(binfile));

	// binary파일이 있다면 그것을 로드한다.
	// binary파일은 같은 파일명에서 확장자만(.bmm) 바뀐다. 
	SMapItem item;
	ZeroMemory( &item, sizeof(item) );
	if (CompareExtendName(szFileName, strlen(szFileName), "bmm") || PathFileExists(binfile))
	{
		// binary파일이 있다면 그것을 로드한다.
...
SAniLoader* CFileLoader::LoadAnimation( char *szFileName )
{
	...
	char binfile[ MAX_PATH];
	ChangeExtendName(szFileName, strlen(szFileName), "a", binfile, sizeof(binfile));

	// binary파일이 있다면 그것을 로드한다.
	// binary파일은 같은 파일명에서 확장자만(.a) 바뀐다. 
	SMapItem item;
	ZeroMemory( &item, sizeof(item) );
	if (CompareExtendName(szFileName, strlen(szFileName), "a") || PathFileExists(binfile))
	{
	...
	else
	{
		...
		strcpy_s( srcfilename, sizeof(srcfilename), szFileName );
		strcat_s( srcfilename, sizeof(srcfilename), "_.txt" );
...
BOOL CFileLoader::ConvertAniListToAniform( char *szSrcFileName, char *szDstFileName )
{
	...
	for( int i=0; i < pani->size; ++i )
	{
		...
		if( 0 < ploader->size )
		{
			...
			strcpy_s( ploader->pAniGroup[ 0].name, sizeof(ploader->pAniGroup[ 0].name), pani->pAniInfo[ i].name );
...
void CFileLoader::ModifyTextureFilename( SMaterialGroupLoader *pLoader )
{
	for( int i=0; i < pLoader->size; ++i )
	{
		if( pLoader->pMtrl[ i].szFileName[0] )
		{
			...
			strcpy_s( szTemp, sizeof(szTemp), &pLoader->pMtrl[ i].szFileName[ k] );
			ZeroMemory( pLoader->pMtrl[ i].szFileName, sizeof(pLoader->pMtrl[ i].szFileName) );
			sprintf_s( pLoader->pMtrl[ i].szFileName, sizeof(pLoader->pMtrl[ i].szFileName), "data//image//%s", szTemp );
		}
	}
}
...
BOOL CFileLoader::CreateIndexBuffer( SBMMLoader *pLoader )
{
	...
	for( int i=0; i < size; ++i )
	{
		...
		if( 0 < pm->i.size )
		{
			...
			SVector3s *pi;
			pm->pidxbuff->Lock( 0, pm->i.size, (void**)&pi, 0 );
			memcpy( pi, pm->i.pI, pm->i.size * sizeof(SVector3s) );
			pm->pidxbuff->Unlock();
		}
	}
...
// [2011/2/25 jjuiddong]
//------------------------------------------------------------------------
BOOL CFileLoader::CreateMeshVertexBuffer( SMeshLoader *pMesh )
{
	int fvf, stride, vsize;
	BYTE *pv;

	if( 0 < pMesh->vn.size ) // FVF = D3DFVF_XYZ | D3DFVF_NORMAL
	{
		fvf = SVtxNorm::FVF;
	...
	else // FVF = D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_TEX1
	{
		...
		vsize = pMesh->vnt.size;
		pv = (BYTE*)pMesh->vnt.pV;
	}
	...
	if( FAILED(g_pDevice->CreateVertexBuffer(stride*vsize, 0, fvf, D3DPOOL_MANAGED, &pMesh->pvtxbuff, NULL)) )
		return FALSE;

	BYTE *v;
	pMesh->pvtxbuff->Lock( 0, 0, (void**)&v, 0 );
...
int CFileLoader::LoadCombo( char *pFileName, SCombo *pReval[] )
{
	...
	char binfile[ MAX_PATH];
	strcpy_s( binfile, sizeof(binfile), pFileName );
	strcpy_s( &binfile[ strlen(binfile)-3], 3, "b" );

//	if( !PathFileExists(binfile) )
	{
		char buf[ 128];
		sprintf_s( buf, sizeof(buf), "script\\comboparser.exe 0 < %s", pFileName );
		system( buf );
	}
	...
	for( int i=0; i < size; ++i )
	{
		...
		while( !que.empty() )
		{
			...
			pact->anitime = (int)((float)pact->anitime * FRAME);
			pact->stime = (int)((float)pact->stime * FRAME);
			pact->etime = (int)((float)pact->etime * FRAME);
			pact->col_stime = (int)((float)pact->col_stime * FRAME);
			pact->col_etime = (int)((float)pact->col_etime * FRAME);
...
SScene* CFileLoader::LoadUI( char *pFileName )
{
	...
	char binfile[ MAX_PATH];
	strcpy_s( binfile, sizeof(binfile), pFileName );
	strcpy_s( &binfile[ strlen(binfile)-3], 3, "ui" );
