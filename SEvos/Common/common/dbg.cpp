...
#define VALISTPARSE(recv,src) {\
								va_list marker;\
								va_start(marker, src);\
								vsprintf_s(recv, sizeof(recv), src, marker);\
								va_end(marker); }
...
void CDbg::PCInfo( int nOutputType, int nLogID ) // nLogID = -1
{
	...
	info.GetOS( szTemp, sizeof(szTemp) );
	strcat_s( szPcInfo, sizeof(szPcInfo), szTemp ); strcat_s( szPcInfo, sizeof(szPcInfo), "\n" );
	info.GetCPUInfo( szTemp, sizeof(szTemp) );
	strcat_s( szPcInfo, sizeof(szPcInfo), szTemp ); strcat_s( szPcInfo, sizeof(szPcInfo),"\n" );
	info.GetVgaInfo( szTemp, sizeof(szTemp) );
	strcat_s( szPcInfo, sizeof(szPcInfo), szTemp ); strcat_s( szPcInfo, sizeof(szPcInfo), "\n" );
	info.GetHddInfo( szTemp, sizeof(szTemp) );
	strcat_s( szPcInfo, sizeof(szPcInfo), szTemp ); strcat_s( szPcInfo, sizeof(szPcInfo), "\n" );
	info.GetIPAddress( szTemp, sizeof(szTemp) );
	strcat_s( szPcInfo, sizeof(szPcInfo), szTemp ); strcat_s( szPcInfo, sizeof(szPcInfo), "\n" );
...
{
	for( int i=0; i < (int)m_TrackingValue.size(); ++i )
