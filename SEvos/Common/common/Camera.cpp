...
void CCamera::SetLookAt( const Vector3& vAt )
{
	m_vAt = vAt;
} //CCmera::SetLookAt
...
//-----------------------------------------------------------------------------//
// 절두체 컬링을 위해 필요한 함수다.
// return value: ViewMatrix * ProjMatrix 
//-----------------------------------------------------------------------------//
void CCamera::GetViewProjMatrix( Matrix44 *pOut )
{
	if( !pOut ) return;
	*pOut = m_matView * m_matProj;
//	*pOut = m_matProj * m_matView;
}
