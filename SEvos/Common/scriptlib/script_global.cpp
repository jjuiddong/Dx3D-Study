		...
		if( IdK == pTree->kind )
		{
			...
			if( FindDefine( pTree->attr.name, &t, &str) )
			{
				...
				else if( STRING == t )
				{
					...
					strcpy_s( pTree->attr.name, str.size() + 1, str.c_str() );
				}
			}
		}
	}
...
BOOL CTypeTable::NewType( STypeData Type )
{
	...
	char key[ 64];
	strcpy_s( key, sizeof(key), Type.name );
...
BOOL CSymTable::AddSymbol( char *pSymbolName, BOOL import, int id ) // import=FALSE, id=0
{
	...
	SSymData sd;
	strcpy_s( sd.name, sizeof(sd.name), pSymbolName );
	sd.type[ 0] = NULL;
//	sd.type = NULL;
	sd.offset = 0;
	sd.import = import;
...
//	p->type = pTypeName;
	if( pTypeName )	strcpy_s( p->type, sizeof(p->type), pTypeName );
	return TRUE;
}
...
char* CSymTable::GetType( char *pSymbolName )
{
	...
	if( pdot )
	{
		// 클래스 멤버변수 or 멤버함수
		char var[ 64];
		strncpy_s( var, sizeof(var), pSymbolName, pdot-pSymbolName );
