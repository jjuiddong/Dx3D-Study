							    ...
							    char *pArgument )
{
	...
	if( pArgument ) strcpy_s( m_pArgument, sizeof(m_pArgument), pArgument );

	char filename[ MAX_PATH];
	strcpy_s( filename, sizeof(filename), pFileName );
	strcpy_s( &filename[ strlen(filename)-3], 3, "gm" );
...
CProcessor::STEPRESULT CProcessor::stepAcu( INSTRUCTION *pcurrentinst )
{
	...
	case opILD: m_Reg[ r] = (float)(*(int*)*(DWORD*)&m_dMem[ m]); break;
	...
	case opJLT: if( m_Reg[ r] < 0 ) m_Reg[ REG_PC] = (float)m; break;
	case opJLE: if( m_Reg[ r] <= 0 ) m_Reg[ REG_PC] = (float)m; break;
	case opJGT: if( m_Reg[ r] > 0 ) m_Reg[ REG_PC] = (float)m; break;
	case opJGE: if( m_Reg[ r] >= 0 ) m_Reg[ REG_PC] = (float)m; break;
	case opJEQ: if( m_Reg[ r] == 0 ) m_Reg[ REG_PC] = (float)m; break;
	case opJNE: if( m_Reg[ r] != 0 ) m_Reg[ REG_PC] = (float)m; break;
	}
...
BOOL CProcessor::ReadInstructions_Binary( char *pFileName )
{
	...
	char filename[ MAX_PATH];
	strcpy_s( filename, sizeof(filename), pFileName );
	strcpy_s( &filename[ strlen(filename)-2], 4, "bgm" );
