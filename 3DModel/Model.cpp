...
//-----------------------------------------------------------------------------//
CModel::CModel() : 
m_pBone(NULL), m_eType(ANI_RIGID), m_nMeshCount(0), m_pAniLoader(NULL), m_fSize(1.0f),
...
BOOL CModel::LoadDynamicModel( SBMMLoader *pLoader, BOOL bCollision ) // bCollision = FALSE
{
	...
	for( int i=0; i < pLoader->m.size; ++i )
	{
		...
		case ANI_SKIN: pMesh = new CAniMesh( p, m_pBone->GetPalette(), FALSE ); break;
		}
		m_pMesh[ i] = pMesh;
	}
...
// Animation
//-----------------------------------------------------------------------------//
BOOL CModel::Animate( int nDelta )
{
	if( m_bUpdateMatrix )
	{
