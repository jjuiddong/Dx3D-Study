...
static Tokentype reservedLookup( char *s )
{
	...
	{
		if( !stricmp(s, reservedWords[ i].str) )
...
char* CScanner::CopyTokenStringQ( int nIdx )
{
	...
	strcpy( p, m_TokQ[ nIdx].str.c_str() );
	return p;
}
...
SParseTree* CParser::Parse( char *szFileName, CPreCompiler *pPreC, BOOL bTrace ) // bTrace=FALSE
{
...
SParseTree* CParser::include_list()
{
...
SParseTree*  CParser::include_stmt()
{
...
SParseTree* CParser::import_list()
{
	SParseTree *t = NULL, *p = NULL;
...
SParseTree* CParser::variable_list()
{
	SParseTree *t = NULL, *p = NULL;
	while( ID == m_Token )
	{
...
SParseTree* CParser::function_list()
{
	SParseTree *t = NULL, *p = NULL;
...
SParseTree* CParser::statement_list()
{
...
SParseTree* CParser::define_list()
{
...
SParseTree* CParser::define_stmt()
{
...
//	assign_stmt -> id = exp
SParseTree* CParser::assign_stmt()
{
...
SParseTree* CParser::compose_stmt()
{
	SParseTree *t;
	if( LBRACE == m_Token )
	{
...
SParseTree* CParser::num()
{
	...
	t->attr.fnum = atof( m_pScan->GetTokenStringQ(0) );
...
BOOL CParser::match( ns_script::Tokentype t )
{
	if( m_Token == t )
	{
		m_Token = m_pScan->GetToken();
...
void CParser::SyntaxError( char *szMsg, ... )
{
	m_bError = TRUE;
	char buf[ 256];
	va_list marker;
	va_start(marker, szMsg);
	vsprintf(buf, szMsg, marker);
	va_end(marker);
	g_Dbg.Console( ">>>" );
