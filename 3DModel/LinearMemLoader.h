...
public:
	...
	BOOL LoadTokenFile( char *szTokenFileName );
	BOOL AddToken( char *szTokenName, int nSize, char *szParentTokenName=NULL );

protected:
	void CreateDefaultToken();
	void LoadTokenFileRec( SDftDataList *pNode, MemberList *pTokenList=NULL );
	...
	void ParseData( BYTE *pStruct, SToken *pToken, char *szToken );
