...
protected:
	...
	{ 
		...
		WriteFile(hOut, text, (DWORD)strlen(text), &dwWrite, NULL);
	}
private:
    std::basic_string<char_type, traits_type> m_buf;
};
...
{
public:
	basic_filebuf1( char *filename ) { strcpy_s(m_filename,filename); }
protected:
    int_type overflow(int_type ch = traits_type::eof())
    {
        if (!traits_type::eq_int_type(traits_type::eof(), ch))
...
public:
	...
	{
		m_debugbuf.sputn( ch, (std::streamsize)strlen(ch) );
		m_debugbuf.pubsync();
		return *this;
	}
};
