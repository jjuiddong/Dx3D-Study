...
public:
	...
	BOOL GetIPAddress( LPSTR lpBuf );
	VOID GetWorkGroupName( LPTSTR szGroupName, DWORD *pdwSize );
	void GetVgaInfo( char *szInfo );
	void GetHddInfo( char *szInfo );	
	...
	BOOL GetComputerName( LPSTR lpBuf );
	BOOL GetDiskSize( LPSTR lpBuf );
