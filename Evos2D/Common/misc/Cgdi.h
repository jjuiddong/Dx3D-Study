...
public:
  ...
  {
    TextOut(m_hdc, x, y, (LPCTSTR)s.c_str(), (int)s.size());
  }

  void TextAtPos(double x, double y, const std::string &s)
  {
    TextOut(m_hdc, (int)x, (int)y, (LPCTSTR)s.c_str(), (int)s.size());
  }

  void TextAtPos(Vector2D pos, const std::string &s)
  {
    TextOut(m_hdc, (int)pos.x, (int)pos.y, (LPCTSTR)s.c_str(), (int)s.size());
  }
  ...
  void SetPenColor(int color)
  {
   ...
   switch (color)
   {
	...
	case thickred: ThickRedPen(); return;
	case thickgreen: ThickGreenPen(); return;
	case thickblue: ThickBluePen(); return;
    }//end switch
  }
};
