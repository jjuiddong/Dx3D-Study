...
template <class graph_type>
void Graph_SearchDijkstra<graph_type>::GetPathToTarget(std::list<int> *plist)const
{
	//just return an empty path if no target or no path found
...
void Graph_SearchAStar<graph_type, heuristic>::Search()
{
  ...
  while(!pq.empty())
  {
         ...
         pE=ConstEdgeItr.next())
    {
      ...
      const double HCost = heuristic::Calculate(m_Graph, m_iTarget, pE->To()); 
...
template <class graph_type, class heuristic>
void Graph_SearchAStar<graph_type, heuristic>::GetPathToTarget(std::list<int> *plist)const
{
	//just return an empty path if no target or no path found
	if (m_iTarget < 0)  return;
