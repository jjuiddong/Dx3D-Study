...
#include "graph/SearchTerminationPolicies.h"
...
public:
  ...
  virtual void							GetPathToTarget(std::list<int> *plist)const=0;

  //returns the path as a list of PathEdges
//  virtual std::list<PathEdge>           GetPathAsPathEdges()const=0;
  virtual void				            GetPathAsPathEdges(std::list<PathEdge> *plist)const=0;
...
public:
  ...
  void					 GetPathToTarget(std::list<int> *plist)const;

  //returns the path as a list of PathEdges
//  std::list<PathEdge>    GetPathAsPathEdges()const;
  void		             GetPathAsPathEdges(std::list<PathEdge> *plist)const;
...
//    double GCost = m_GCosts[NextClosestNode] + pE->Cost();
	//calculate the 'real' cost to this node from the source (G)
	const NavGraphNode<> &t = m_Graph.GetNode(pE->To());
	const NavGraphNode<> &f = m_Graph.GetNode(pE->From());
	const double cost = t.Pos().Distance(f.Pos());
	//	  const double cost = pE->Cost();
	const double GCost = m_GCosts[NextClosestNode] + cost;
...
template <class graph_type, class heuristic>
void Graph_SearchAStar_TS<graph_type, heuristic>::GetPathToTarget(std::list<int> *plist)const
{
...
template <class graph_type, class heuristic>
void Graph_SearchAStar_TS<graph_type, heuristic>::GetPathAsPathEdges(std::list<PathEdge> *plist)const
{
	//just return an empty path if no target or no path found
	if (m_iTarget < 0)  return;
	int nd = m_iTarget;
	while ((nd != m_iSource) && (m_ShortestPathTree[nd] != 0))
	{
		plist->push_front(PathEdge(m_Graph.GetNode(m_ShortestPathTree[nd]->From()).Pos(),
			m_Graph.GetNode(m_ShortestPathTree[nd]->To()).Pos(),
			m_ShortestPathTree[nd]->Flags(),
			m_ShortestPathTree[nd]->IDofIntersectingEntity()));
...
template <class graph_type, class termination_condition>
void Graph_SearchDijkstras_TS<graph_type, termination_condition>::GetPathToTarget(std::list<int> *plist)const
{
	//just return an empty path if no target or no path found
	if (m_iTarget < 0)  return;    
	int nd = m_iTarget;
...
template <class graph_type, class termination_condition>
void Graph_SearchDijkstras_TS<graph_type, termination_condition>::GetPathAsPathEdges(std::list<PathEdge> *plist)const
{
	//just return an empty path if no target or no path found
	if (m_iTarget < 0)  return;
	int nd = m_iTarget;
