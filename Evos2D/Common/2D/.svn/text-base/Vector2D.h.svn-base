...
struct Vector2D
{
  ...
  Vector2D  Interpolation(const Vector2D &v2, const double val) const;
  Vector2D  Interpolation(const Vector2D &v1, const Vector2D &v2, const double val) const;

  // 점과 직선사이 거리
  double	   DistanceLineToPoint(const Vector2D &v1, const Vector2D &v2, const Vector2D &point) const;
